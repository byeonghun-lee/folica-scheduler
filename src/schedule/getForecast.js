    ...
    try {
        ...
        for (let index = 0; index < weatherAlarmList.length; index++) {
            ...
            try {
                ...
                console.log("Process alarm error:", processError);
                console.log("target:", weatherAlarmItem);
            } finally {
                // 성공/실패 무관하게 항상 nextAlertDate 업데이트
                try {
                    if (weatherAlarmItem.dayOfTheWeek?.length > 0) {
                        const nextAlertDayOfWeek = getNextClosestDay({
                            ...
                            alertDaysBefore:
                                weatherAlarmItem.alertDaysBefore,
                            alertTime: weatherAlarmItem.alertTime,
                        });
                        ...
                        weatherAlarmItem.nextAlertDate = dayjs()
                            ...
                            .hour(
                                dayjs(weatherAlarmItem.alertTime).hour()
                            )
                            .minute(
                                dayjs(weatherAlarmItem.alertTime).minute()
                            )
                            .startOf("second")
                            .toDate();
                    } else {
                    ...
                    console.log("Save nextAlertDate error:", saveError);
                }
            }
        }
        console.log("END FORECAST.");
